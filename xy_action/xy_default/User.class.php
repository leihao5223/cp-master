...
class User extends WebBase {
   ...
   private function getTimeSetting($key, $default = 0) {
    ...
    public final function logined() {
        ...
        // 把别人踢下线
        $sql="select * from {$this->prename}member_session  where uid={$user['uid']} and id<{$user['sessionId']}";
        $rel=$this->getRow($sql);
       if($rel!=false){
        $re=$this->update("update {$this->prename}member_session set isOnLine=0,state=1 where uid={$user['uid']} and id<{$user['sessionId']}");
        }
        return $user;
    }
    ...
    public final function inAgent() {
        ...
        if (!ctype_digit($para['amount']))
            throw new Exception('销量包含非法字符，请填入纯数字');
        if (!ctype_digit($para['qq']))
            throw new Exception('QQ号码包含非法字符');
        if (strlen($para['name']) > 40)
            throw new Exception('平台名请不要超过40个字符');

        $para['time'] = $this->time;
        $para['ip'] = $this->ip(true);
        if (!$this->insertRow($this->prename . 'inagent', $para))
    ...
    public final function reg() {
        ...
        if (strlen($user) < 6 || strlen($user) > 16)
            throw new Exception('用户名为6-16位,请重新输入');
        ...
        if (!$link = $this->getRow("select * from {$this->prename}links where lid=?", $lid)) {
            ...
            try {
                ...
                if ($this->getValue($sql, $para['username']))
                    throw new Exception('用户"' . $para['username'] . '"已经存在');
